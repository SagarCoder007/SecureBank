...
export function exportAccountsToCSV(accounts: any[]): void {
  const csvData = accounts.map(account => ({
    ...
    'Last Activity': account.lastTransaction 
      ? new Date(account.lastTransaction.createdAt).toLocaleDateString()
      : 'No activity'
  }));
...
export function exportTransactionsToCSV(transactions: any[]): void {
  const csvData = transactions.map(tx => ({
    ...
    'Balance After': `$${parseFloat(tx.balanceAfter).toFixed(2)}`,
...
export function exportAccountsToPDF(accounts: any[]): void {
...
function generateAccountsHTML(accounts: any[]): string {
  ...
  return `
    ...
    <body>
      ...
      <div class="summary">
        ...
        <p><strong>Total Balance:</strong> $${accounts.reduce((sum, a) => sum + parseFloat(a.balance), 0).toFixed(2)}</p>
      </div>
...
export function filterAccounts(accounts: any[], filters: {
  search?: string;
